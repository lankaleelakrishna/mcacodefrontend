...
export default function Login() {
  ...
  // Customer-only login (no role selector)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = `${API_BASE}/customer/login`;
      ...
      login(data.token);
      // Default post-login for customer: go to home
      navigate('/');
    } catch (err) {
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            {/* Customer login only - role is fixed on backend endpoint */}
